...
import { Incentive, Position, Stake, Unstake, Claim, Gauge } from '../generated/schema';
...
export function handleClaimRewards(event: ClaimRewards): void {
  let gauge = Gauge.load(event.address.toHex())
  if (gauge) {
    gauge.claimTotal = gauge.claimTotal.plus(event.params.amount);
    gauge.save();
...
export function handleNotifyReward(event: NotifyReward): void {
  ...
  if (gauge) {
    ...
    if (endTime.reverted || rewardRate.reverted) {
